...
# calculate summary statistics for all participants
  tableCS <- std_subScale8 %>% summarize(
    ...
    # non-standardized, original subscales scores
    meanPF = mean(PF), sdPF = sd(PF), sePF = sd(PF)/sqrt(n()),
    meanRP = mean(RP), sdRP = sd(RP), seRP = sd(RP)/sqrt(n()),
    meanBP = mean(BP), sdBP = sd(BP), seBP = sd(BP)/sqrt(n()),
    meanGH = mean(GH), sdGH = sd(GH), seGH = sd(GH)/sqrt(n()),
    meanVT = mean(VT), sdVT = sd(VT), seVT = sd(VT)/sqrt(n()),
    meanSF = mean(SF), sdSF = sd(SF), seSF = sd(SF)/sqrt(n()),
    meanRE = mean(RE), sdRE = sd(RE), seRE = sd(RE)/sqrt(n()),
    meanMH = mean(MH), sdMH = sd(MH), seMH = sd(MH)/sqrt(n()),
