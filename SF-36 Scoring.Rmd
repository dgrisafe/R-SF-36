...
colnames(finalTable) <- c("mean", "stdDev", "Scale", "Abbreviation", "weight")

# formatted final table
finalTable <- finalTable %>% select(Scale, Abbreviation, weight, mean, stdDev)
...
## present summary statistics of standardized scores in tibble
  ...
  std_sumScale8 <- summarize(std_subScale8, 
            meanstdPF = mean(stdPF), sdPF = sd(stdPF),
            meanstdRP = mean(stdRP), sdRP = sd(stdRP),
            meanstdBP = mean(stdBP), sdBP = sd(stdBP),
            meanstdGH = mean(stdGH), sdGH = sd(stdGH),
            meanstdVT = mean(stdVT), sdVT = sd(stdVT),
            meanstdSF = mean(stdSF), sdSF = sd(stdSF),
            meanstdRE = mean(stdRE), sdRE = sd(stdRE),
            meanstdMH = mean(stdMH), sdMH = sd(stdMH)
            )
  ...
  # name columns and reorder columns
  colnames(std_finalTable) <- c("mean", "stdDev", "Scale", "Abbreviation", "weight")
  
  # formatted final table
  std_finalTable <- std_finalTable %>% select(Scale, Abbreviation, weight, mean, stdDev)
